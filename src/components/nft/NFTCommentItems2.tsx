...
import RenderReplies from "../ai/renderReplies";
...
const NFTCommentsPage: React.FC = () => {
    ...
    try {
      const response = await fetch(
        "https://toptop-api-facbf95cbd23.herokuapp.com/comments/comments/newest/pages/cat",
        {
          ...
          body: JSON.stringify({ pageNumber: pageNum || 0, cat: "NFT" }),
        }
      );
  ...
  commentElements.forEach((commentElement) => {
    commentElement.addEventListener("click", () => {
      ...
      window.location.href = `/nft/item/${itemId}`;
    });
  });
  ...
  repls.forEach((element) => {
    element.addEventListener("click", () => {
      ...
      window.location.href = `/nft/comments-selected${commentId}`;
    });
  });
...
export default NFTCommentsPage;
