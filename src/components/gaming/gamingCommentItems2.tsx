...
const GamingCommentsPage: React.FC = () => {
    ...
    try {
      const response = await fetch(
        "https://toptop-api-facbf95cbd23.herokuapp.com/comments/comments/newest/pages/cat",
        {
          ...
          body: JSON.stringify({ pageNumber: pageNum || 0, cat: "Gaming" }),
        }
      );
  ...
  commentElements.forEach((commentElement) => {
    commentElement.addEventListener("click", () => {
      ...
      window.location.href = `/gaming/item/${itemId}`;
    });
  });
  const handleDeleteComment = async (id: string) => {
    try {
      const response = await fetch(`https://toptop-api-facbf95cbd23.herokuapp.com/comments/${id}`, {
        method: "DELETE",
  ...
  repls.forEach((element) => {
    element.addEventListener("click", () => {
      ...
      window.location.href = `/gaming/comments/selected/${commentId}`;
    });
  });
...
export default GamingCommentsPage;
