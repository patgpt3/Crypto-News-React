...
async function fetchData(pageNum) {
  try {
    const response = await fetch("https://crypto-api-3-6bf97d4979d1.herokuapp.com/items/ask/pages", {
...
async function main() {
  ...
  const pageNumberInput = localStorage.getItem("pageNumberAsk");
  ...
  if (users.length < 30) {
    ...
    const moreButton = document.getElementById("next-page-ask");
    moreButton.style.display = "none";
  }

  if (numberInput === 0) {
    //hide back
    const moreButton = document.getElementById("back-page-ask");
    moreButton.style.display = "none";
  }
  ...
  const addNEXTButtonEvent = document.getElementById("next-page-ask");
  addNEXTButtonEvent.addEventListener("click", () => {
    const page = localStorage.getItem("pageNumberAsk");

    const num = parseInt(page) === NaN ? 0 : parseInt(page);

    localStorage.setItem("pageNumberAsk", num ? num + 1 : 1);
    window.location.href = "/crypto/ask";
  });

  // Add event listener for the change email button
  const addbackButtonEvent = document.getElementById("back-page-ask");
  addbackButtonEvent.addEventListener("click", () => {
    const page = localStorage.getItem("pageNumberAsk");

    const num = parseInt(page);

    localStorage.setItem("pageNumberAsk", num - 1);
    window.location.href = "/cryptoAsk";
  });
  // // Add click listeners to .cnUser elements
  // const elements = document.querySelectorAll(".cnUser");
  // elements.forEach((element) => {
